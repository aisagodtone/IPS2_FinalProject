...
void UI::draw() {
    ...
    int raw_key_num = DC->player->max_key;
    int key_num     = std::clamp(raw_key_num, 0, 3); // 對應 key_imgs[0..2]

    if (!love || !key_imgs[0] || !no_key || !alert) {
        debug_log("[UI::draw] ERROR: some UI bitmaps are NULL, skip HUD\n");
        return;
    }
    ...
    for (int i = 0; i < key_num; i++) {
        float x = game_field_length - (key_width + 20) * (i + 1);
        float y = 30 + love_height;

        if (have_key[i]) {
            if (key_imgs[i]) {
                al_draw_bitmap(key_imgs[i], x, y, 0);
            }
        } else {
            al_draw_bitmap(no_key, x, y, 0);
        }
    }

    // 警戒 icon
    for (int i = 0; i < alert_level; i++) {
        float x = 10 + alert_width * (i + 1);
        float y = 30 + love_height;
        al_draw_bitmap(alert, x, y, 0);
    }
}
